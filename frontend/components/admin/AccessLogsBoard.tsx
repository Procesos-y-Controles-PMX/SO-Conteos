...
const APP_OPTIONS = [
  ...
  { id: "equipo", label: "Equipo Móvil" },
  { id: "cotizador", label: "Cotizador" },
  { id: "permisos", label: "Permisos" },
  { id: "carta-responsiva", label: "Cartas Responsivas" },
...
export default function AccessLogsBoard({ endpoint = "/api/admin/access-logs" }: { endpoint?: string }) {
  ...
  }, [endpoint, rangeDays, search, appFilter]);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
        <div className="neu-raised rounded-lg p-4">
          <span className="text-[10px] font-bold uppercase tracking-wider text-fg-faint">Accesos</span>
          <p className="mt-1 font-display text-2xl font-semibold text-fg">{loading ? "…" : total}</p>
        </div>
        <div className="neu-raised rounded-lg p-4">
          <span className="text-[10px] font-bold uppercase tracking-wider text-fg-faint">Usuarios únicos</span>
          <p className="mt-1 font-display text-2xl font-semibold text-brand">{loading ? "…" : uniqueUsers}</p>
        </div>
        <div className="neu-raised col-span-2 rounded-lg p-4 md:col-span-1">
          <span className="text-[10px] font-bold uppercase tracking-wider text-fg-faint">Días con actividad</span>
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end">
          <div className="min-w-0 flex-1">
            ...
            <div className="relative">
              <Search
                size={15}
                className="pointer-events-none absolute left-3.5 top-1/2 z-10 -translate-y-1/2 text-fg-faint"
              />
              <input
                value={searchInput}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {visibleDays.map((day) => {
            ...
            return (
              <div key={day.date} className="neu-raised overflow-hidden rounded-lg">
                <button
                  type="button"
